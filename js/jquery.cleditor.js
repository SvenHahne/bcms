...
(function ($) {
  ...
  function showPopup(editor, popup, button) {
    ...
    hidePopups();
                    $popup.css({left: left, top: top, zIndex:99999})
      .show();
  ...
  function updateTextArea(editor, checkForChange) {
   ...
   // console.log("update updateTextArea");
    var html = $(editor.doc.body).html(),
      options = editor.options,
      updateTextAreaCallback = options.updateTextArea,
      $area = editor.$area;

    // remove formatting, only keep tags p, br, b, u, i, a, h2 (---hack)
    html = html.replace(/<(?!br|p|\/p|hr|\/hr|sup|\/sup|b|\/b|u|\/u|i|\/i|a|\/a|font|\/font|h2|\/h2\s*\/?)[^>]+>/g, " ");
    // remove attributes
   // html = html.replace(/<(?!a)\s*(\w+).*?>/g, '<$1>');
